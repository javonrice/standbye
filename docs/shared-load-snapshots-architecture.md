...
Status: **MVP implementation in progress** — product decisions below are locked. Vision provider is Gemini Flash direct with a Lovable drop-in adapter (`LOAD_SCREENSHOT_PROVIDER=lovable`).  
...
**Optional Lovable drop-in:** set `LOAD_SCREENSHOT_PROVIDER=lovable` with `LOVABLE_VISION_URL` + `LOVABLE_VISION_API_KEY`. Same `LoadScreenshotParser` contract — product code does not import Lovable elsewhere.
...
8. Vision is Gemini Flash direct by default; Lovable is an optional env drop-in (`LOAD_SCREENSHOT_PROVIDER=lovable`).  
